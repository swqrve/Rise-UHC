    ...
    public DiamondMenu(Player p) {
        ...
        for (int i = 0; i < neededPages; i++) {
            ...
            for (int b = 0; b < 18; b++) {
                ...
                page.put(b, new ItemCreator(Material.SKULL_ITEM, 1).setName("&6" + player.getPlayerObject().getDisplayName()).addLore(Arrays.asList("&bDiamonds &7Mined: " + player.getCurrentDiamondsMined(), "&7Click to teleport to the player!")).setOwner(player.getPlayerObject().getName()).getItem());
