package me.swerve.riseuhc.command;

import me.swerve.riseuhc.RiseUHC;
import me.swerve.riseuhc.menu.board.ConfigMenu;
import me.swerve.riseuhc.util.CrashType;
import me.swerve.riseuhc.util.CrashUtils;
import org.bukkit.Bukkit;
import org.bukkit.ChatColor;
import org.bukkit.command.Command;
import org.bukkit.command.CommandExecutor;
import org.bukkit.command.CommandSender;
import org.bukkit.entity.Player;

public class CrashCommand implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        Player p = (Player) sender;
        if (!p.hasPermission("uhc.crash")) {
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f[&6RiseUHC&f] &fNo Permission."));
            return false;
        }

        if (args.length != 2) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cUsage: /crash <name> <explosion | position | all>"));
            return false;
        }

        String playerName = args[0];

        if (Bukkit.getPlayer(playerName) == null) {
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cCould not find a player by that name."));
            return false;
        }

        if (!Bukkit.getPlayer(playerName).isOnline()) {
            p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cThat player is not available!"));
            return false;
        }

        if (args[1].equalsIgnoreCase("all")) {
            for (CrashType type : CrashType.values()) CrashUtils.crashPlayer(sender, Bukkit.getPlayer(args[0]), type);
            return true;
        }

        CrashType type = CrashType.fromString(args[1]);
        if (type == null) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cThat method doesn't exist."));
            return true;
        }

        CrashUtils.crashPlayer(sender, Bukkit.getPlayer(args[0]), type);
        return false;
    }
}

